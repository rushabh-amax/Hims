(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function initSidebarToggleButton2() {
      const currentPath = window.location.pathname;
      const appRootRouteRegex = /^\/app\/[^/]+$/;
      if (!appRootRouteRegex.test(currentPath)) {
        return;
      ...
      const pageHead = document.querySelector(".page-head-content");
      if (!sidebar || !pageHead) {
        setTimeout(initSidebarToggleButton2, 200);
        return;
      }
      if (pageHead.querySelector(".custom-sidebar-toggle"))
        return;
      ...
      toggleButton.addEventListener("click", function() {
        isExpanded = !isExpanded;
        if (isExpanded) {
          expandSidebar2(sidebar);
        } else {
          collapseSidebar2(sidebar);
        }
      });
    }
    frappe.after_ajax(() => {
      initSidebarToggleButton2();
    });
    function collapseSidebar2(sidebar) {
      sidebar.classList.add("w-60px", "px-2");
      sidebar.classList.remove("col-lg-2", "col-md-3");
      document.querySelectorAll(".standard-sidebar-item .sidebar-item-label").forEach((el) => {
        el.classList.add("d-none");
    ...
    function expandSidebar2(sidebar) {
      sidebar.classList.remove("col-auto");
      sidebar.classList.add("col-lg-2", "col-md-3");
      document.querySelectorAll(".standard-sidebar-item .sidebar-item-label").forEach((el) => {
        el.classList.remove("d-none");
    ...
    document.addEventListener("DOMContentLoaded", initSidebarToggleButton2);
    function movePageHeadContent2() {
    ...
    document.addEventListener("DOMContentLoaded", movePageHeadContent2);
    document.addEventListener("DOMContentLoaded", function() {
      function initSlidingTabs(ul) {
        if (!ul || ul.dataset.slidingTabsInitialized)
          return;
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.querySelectorAll('input[type="password"]').forEach(injectToggle2);
      const observer2 = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          mutation.addedNodes.forEach((node) => {
            var _a, _b;
            if (node.nodeType === 1) {
              if ((_a = node.matches) == null ? void 0 : _a.call(node, 'input[type="password"]')) {
                injectToggle2(node);
              }
              (_b = node.querySelectorAll) == null ? void 0 : _b.call(node, 'input[type="password"]').forEach(injectToggle2);
            }
          });
        });
      });
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          ...
          if (node.nodeType === 1) {
            ...
            (_b = node.querySelectorAll) == null ? void 0 : _b.call(node, 'input[type="password"]').forEach(injectToggle);
          }
        });
      });
    });
...
//# sourceMappingURL=customJs.bundle.TRG3ENBV.js.map
